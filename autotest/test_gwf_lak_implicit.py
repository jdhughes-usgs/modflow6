...
def _write_implicit(test, builder, force_legacy=False):
    # build and write the implicit simulation in the test workspace, enabling
    # the IMPLICIT option (and optionally the forced legacy substitution solve)
...
def _enable_dev_legacy(lak_file):
    # add the hidden DEV_FORCE_LEGACY option to the LAK OPTIONS block, which
    # routes every active lake through the legacy substitution path. It is a
...
def test_two_lakes_outlet_forced_legacy(function_tmpdir, targets):
    # the two-lake outlet model with every lake forced onto the legacy solver
    # (DEV_FORCE_LEGACY). That path handles the lake-to-lake outlet routing (it
    # zeroes and recomputes simoutrate before assembling the legacy-solved
    # lakes), so it must reproduce the legacy result exactly. The strict
    # 1e-6 agreement is checked here rather than through the framework's looser
    # head comparison.
    def build(test):
        sim_f, _ = _write_implicit(test, _build_twolake, force_legacy=True)
        return sim_f, None
    ...
    def check(test):
        ...
        sl = _stages(str(ws_l), "lk")
        sf = _stages(ws_f, "lk")
        assert np.allclose(sl, sf, atol=1e-6), f"legacy stage mismatch: {sl} vs {sf}"
        hl = _heads(str(ws_l), "lk")
        hf = _heads(ws_f, "lk")
        assert float(np.nanmax(np.abs(hl - hf))) < 1e-6, "legacy head mismatch"

    _framework(function_tmpdir, targets, build, check, compare=None).run()


@pytest.mark.developmode
def test_weak_lake_three_ways(function_tmpdir, targets):
    ...
    #   3. the IMPLICIT formulation with every lake forced onto the legacy
    #      solver (DEV_FORCE_LEGACY).
    # case 3 routes the lake through the legacy assembly in lak_fc_implicit
    ...
    # not stall the implicit solver on its own, so that path is forced here to
    # give a deterministic regression test of the assembly. The strict
    ...
    def check(test):
        ...
        # IMPLICIT with every lake forced onto the legacy solver
        ws_fb = test.workspace / "forced_legacy"
        ...
        _enable_dev_legacy(str(ws_fb / "lk.lak"))
        assert _run(sim_f), "IMPLICIT with forced legacy solve failed for the weak lake"
        _assert_budget_closes(str(ws_fb), "lk")

        hl = _heads(str(ws_l), "lk")
        sl = _stage(str(ws_l), "lk")
        for label, ws in (("implicit", ws_i), ("forced legacy", str(ws_fb))):
            hx = _heads(ws, "lk")
