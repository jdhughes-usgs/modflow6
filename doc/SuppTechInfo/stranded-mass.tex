...
Simulated concentrations rise as the cell drains in every case, because the water that remains occupies a smaller volume. Without sorption or decay (fig.~\ref{fig:strandedmassnosorption}\textit{A}), the peak concentration reaches 163 grams per cubic meter with the current approach and 126 grams per cubic meter when the option is used, a reduction of 23 percent, because the solute in the water that does not drain is held out of the mobile domain instead of being carried by the water that remains. The mass held out of the mobile domain (fig.~\ref{fig:strandedmassnosorption}\textit{C}) rises and falls with the water table, reaching about 3,640 kilograms at the driest part of each cycle and returning to zero when the cell refills. Successive cycles repeat, because the mass held back in one cycle is returned in the next.

The solute mass of the model (fig.~\ref{fig:strandedmassnosorption}\textit{B}) shows the effect more directly than the concentration does, because a concentration reflects both the mass in the cell and the volume of water that holds it. The model has no source of solute, and the constant head carries solute out of it at a concentration of zero, so the mass of the model can only decrease. With the current approach it does not: the mass rises and falls with the water table, between about 5,100 and 20,600 kilograms, because mass is created while the water table rises and lost while it falls. With the option the mass decreases smoothly from 14,900 to 9,400 kilograms, which is the solute that left through the constant head.

The separation between the two curves narrows over the simulation, and it is worth being clear about why, because the reason is not that the approaches agree better as the simulation proceeds. Solute is leaving the model throughout, so there is progressively less of it for the two approaches to disagree about. Measured against the mass that remains rather than against the mass the simulation started with, the disagreement is nearly the same in the fourth cycle as in the first: the model that uses the option holds about 103 percent more solute than the model that does not at the peak of the first cycle and about 83 percent more at the peak of the fourth. The oscillation is a property of the formulation and persists for as long as the water table moves.

\begin{figure}[!ht]
	\begin{center}
	\includegraphics{./Figures/MSTStrandedMassNoSorption.pdf}
	\caption[Graphs comparing simulated concentrations with and without the stranded mass option, without sorption, for the oscillating water table example]{Graphs comparing simulated concentrations with and without the stranded mass option of the MST Package for the oscillating water table example shown in figure~\ref{fig:strandedmasshead}, without sorption and without decay. The cell has a mobile-domain porosity of 0.3, a specific yield of 0.15, and an initial concentration of 100 grams per cubic meter. \textit{A}, Simulated concentration; \textit{B}, solute mass in the model, which is the sum of the dissolved, sorbed, and stranded mass of both cells; \textit{C}, mass held out of the mobile domain}
	\label{fig:strandedmassnosorption}
	\end{center}
\end{figure}

Adding sorption (fig.~\ref{fig:strandedmasssorption}\textit{A}) lowers the peak concentration to 146 grams per cubic meter, because sorbate buffers the dissolved concentration, and the option lowers it further to 100 grams per cubic meter, a reduction of 31 percent, because the sorbate that the cell gives up as it drains is held back along with the solute in the retained water. With first-order decay active, concentrations decrease from cycle to cycle as mass is lost from the simulation, and the concentration at the end of the simulation falls from 14.4 to 9.9 grams per cubic meter when the option is used. The mass held out of the mobile domain (fig.~\ref{fig:strandedmasssorption}\textit{C}) reaches about 6,780 kilograms in each cycle without decay, roughly twice the amount held back when only the retained water contributes, and decreases from cycle to cycle when decay is active, both because the concentration in the cell is lower and because the stranded mass decays while it is held. The solute mass of the model (fig.~\ref{fig:strandedmasssorption}\textit{B}) oscillates between about 7,600 and 32,400 kilograms with sorption and the current approach, a wider swing than without sorption, because the sorbate that the cell gives up as it drains adds to the solute of the retained water. With the option the mass decreases smoothly. Measured against the mass remaining in the model, the disagreement between the two approaches holds near 115 percent through all four cycles, both with and without decay; decay lowers the mass of every case but does not reduce the disagreement between them.
